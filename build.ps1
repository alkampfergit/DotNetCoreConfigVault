param(
    ...
    [bool]      $forceInsatllBuildUtils = $true
)

if ($forceInsatllBuildUtils)
{
...
dotnet restore "$runningDirectory/src/DotNetCoreConfigVault.sln"
dotnet test "$runningDirectory/src/DotNetCoreConfigVault.Tests/DotNetCoreConfigVault.Tests.csproj" /p:CollectCoverage=true /p:CoverletOutput=TestResults/ /p:CoverletOutputFormat=lcov
dotnet build "$runningDirectory/src/DotNetCoreConfigVault.sln" --configuration release
dotnet pack "$runningDirectory/src/DotNetCoreConfigVault.Client/DotNetCoreConfigVault.Client.csproj" --configuration release -o "$runningDirectory/artifacts/NuGet" /p:PackageVersion=$nugetPackageVersion /p:AssemblyVersion=$assemblyVer /p:FileVersion=$assemblyFileVer /p:InformationalVersion=$assemblyInformationalVersion
